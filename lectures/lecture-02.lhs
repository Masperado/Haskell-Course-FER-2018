...
> prviprvi s = reverse $ drop 3 $ reverse $ drop 3 s
...
> prvitreci s1 s2 = if length s1 > length s2 then s1 ++ s2 else s2 ++ s1
...
> safeHead s = if (null s) then [] else [head s]
...
> shiftLetter x n
>    | ((ord $ toLower x) + n) < ord 'a' = intToChar ((ord $ toLower x) + n + 26)
>    | ((ord $ toLower x) + n) > ord 'z' = intToChar ((ord $ toLower x) + n - 26)
>    | otherwise = intToChar ((ord $ toLower x) + n)

> ceaserCode n xs = [shiftLetter x n | x <- xs, isAlpha x]
...
> letterCount s = totalLength $ [x | x <- words s, length x>=3]

3.2
- Redefine 'isPalindrome' so that it's case insensitive and works correctly 
  for strings that contain whitespaces.

> isPalindrome s = (upperCase $ concat [x | x <- words s]) == (upperCase $ reverse $ concat [x | x <- words s])
...
> flipp xss = concat [reverse x | x <- reverse xss]
...
> inCircle r x y resolution = [(a,b) | a <- resolution, b <- resolution, (x-a)*(x-a)+(y-b)*(y-b)<r*r]
...
> indices x xs = [ i - 1  | (i,a) <- index xs, a==x]
...
> showLineNumbers s = concat[show i ++ " " ++ l ++ "\n" | (i,l) <- zip [1..] $ lines s]
...
> haveAligment xs ys = or[a == b | a <- index xs, b <- index ys]
> common xs ys = [snd a | a <- index xs, b <- index ys, a==b]
