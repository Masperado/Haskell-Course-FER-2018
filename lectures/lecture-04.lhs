...
-- >  shoutOutLoud :: String -> String
-- >  shoutOutLoud (x:xs) = (take 3 $ repeat x) ++ xs
...
> pad s1 s2 = (s1 ++ (take (longest - length s1) $ repeat ' '),s2 ++ (take (longest - length s2) $ repeat ' '))
>   where longest = max (length s1) $ length s2
...
> quartiles :: [Int] -> (Double,Double,Double)
> quartiles xs = (median fs, med, median ls)
>   where  ys = sort xs
>          med = median ys
>          fs = delete' med $ fst $ splitAt (length ys `div` 2) ys
>          ls = delete' med $ snd $ splitAt (length ys `div` 2) ys

> delete' :: Double -> [Int] -> [Int]
> delete' deleted xs = [ x | x <- xs, fromIntegral x /= deleted ]
...
> pad2 s1 s2 = let longest = max (length s1) $ length s2 in
>   (s1 ++ (take (longest - length s1) $ repeat ' '),s2 ++ (take (longest - length s2) $ repeat ' '))
...
> describeTask p xs = "The pair " ++ (case p of
>   (1,1) -> "contains two ones"
>   (1,_) -> "contains one one"
>   (_,1) -> "conatins one one"
>   p -> "does not contain a single one")
>   ++ " and the second element of the list is " ++ (show $ xs !! 2)
